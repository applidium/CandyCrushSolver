...
public abstract class CameraGLRendererBase implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        synchronized(this) {
            ...
            if(texListener != null) {
                ...
                Timber.d("texCamera(OES) -> screen");
                // texCamera(OES) -> screen
                drawTex(texCamera[0], true, 0);
            }
            //Timber.i("onDrawFrame end");
        }
    }

    @Override
    public void onSurfaceChanged(GL10 gl, int surfaceWidth, int surfaceHeight) {
        Timber.i("onSurfaceChanged("+surfaceWidth+"x"+surfaceHeight+")");
        mHaveSurface = true;
        updateState();
    ...
    private void initShaders() {
        ...
        if (strGLVersion != null)
            Timber.i("OpenGL ES version: " + strGLVersion);
    ...
    private static int loadShader(String vss, String fss) {
        Timber.d("CameraGLRendererBase", "loadShader");
        ...
        if (status[0] == 0) {
            Timber.e("CameraGLRendererBase", "Could not compile vertex shader: "+GLES20.glGetShaderInfoLog(vshader));
        ...
        if (status[0] == 0) {
            Timber.e("CameraGLRendererBase", "Could not compile fragment shader:"+GLES20.glGetShaderInfoLog(fshader));
            GLES20.glDeleteShader(vshader);
        ...
        if (status[0] == 0) {
            Timber.e("CameraGLRendererBase", "Could not link shader program: "+GLES20.glGetProgramInfoLog(program));
            program = 0;
            return 0;
        }
        ...
        if (status[0] == 0)
        {
            Timber.e("CameraGLRendererBase", "Shader program validation error: "+GLES20.glGetProgramInfoLog(program));
            GLES20.glDeleteProgram(program);
            program = 0;
            return 0;
        }

        Timber.d("CameraGLRendererBase", "Shader program is built OK");
    ...
    {
        Timber.d("deleteFBO("+mFBOWidth+"x"+mFBOHeight+")");
    ...
    {
        Timber.d("initFBO("+width+"x"+height+")");
        ...
        Timber.d("initFBO error status: " + GLES20.glGetError());
